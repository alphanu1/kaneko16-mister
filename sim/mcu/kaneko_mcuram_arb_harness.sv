...
module kaneko_mcuram_arb_harness #(
    ...
    parameter int unsigned NM     = 3
) (
    input  wire clk,
    input  wire rst_n,

    input  wire            m0_req, m1_req, m2_req,
    input  wire [SDR_AW:1] m0_addr, m1_addr, m2_addr,
    input  wire            m0_we,  m1_we,
    input  wire [15:0]     m0_din, m1_din,
    output logic           m0_ack, m1_ack, m2_ack,
  ...
  wire [NM-1:0]           m_req  = {m2_req, m1_req, m0_req};
  wire [NM-1:0][SDR_AW:1] m_addr = {m2_addr, m1_addr, m0_addr};
  // Master 2 is the data ROM fetch: a reader, and its write signals are tied
  // off here exactly as they are in the core.
  wire [NM-1:0]           m_we   = {1'b0, m1_we, m0_we};
  wire [NM-1:0][15:0]     m_din  = {16'd0, m1_din, m0_din};
  wire [NM-1:0][1:0]      m_be   = {2'b11, 2'b11, 2'b11};
  wire [NM-1:0]           m_ack;

  assign {m2_ack, m1_ack, m0_ack} = m_ack;
