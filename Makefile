...
HARNESSES := kaneko_tmap:kaneko_tmap_layer:sim/video/tb_kaneko_tmap.cpp \
             ...
             kaneko_hit2:kaneko_hit2:sim/io/tb_kaneko_hit2.cpp \
             kaneko_vmem:kaneko_vmem:sim/mem/tb_kaneko_vmem.cpp \
             kaneko_sdram:kaneko_sdram_harness:sim/mem/tb_kaneko_sdram.cpp \
             kaneko_sdram_x2:kaneko_sdram_x2_harness:sim/mem/tb_kaneko_sdram_x2.cpp \
             ...
             kaneko_calc3_keys:kaneko_calc3_keys:sim/mcu/tb_kaneko_calc3_keys.cpp \
             ...
             kaneko_mcuram_arb:kaneko_mcuram_arb_harness:sim/mcu/tb_kaneko_mcuram_arb.cpp \
             kaneko_calc3_sys:kaneko_calc3_sys_harness:sim/mcu/tb_kaneko_calc3_sys.cpp \
             kaneko_ramtest:kaneko_ramtest:sim/mcu/tb_kaneko_ramtest.cpp \
...
quartus: qsf-check nports-check lint test quartus-check
	...
	@# first 96 MHz SDRAM attempt produced one at -3.023 ns without a murmur.
	...
	@# setup by tens of nanoseconds". The SDC guard catches a design that is
	@# not timed at all; this catches one that is timed and fails.
	@slack=$$(grep -oE "Worst-case setup slack is -?[0-9.]+" build/quartus_sta.log \
	          | tail -1 | grep -oE '\-?[0-9.]+$$'); \
	  if [ -z "$$slack" ]; then \
	    echo "quartus: could not read setup slack from build/quartus_sta.log"; \
	    exit 1; fi; \
	  case "$$slack" in -*) \
	    echo "quartus: TIMING FAILED, worst-case setup slack $$slack ns"; \
